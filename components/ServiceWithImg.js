...
const ServiceWithImg = ({ data: { services, isImageSlide } }) => {
  ...
  useEffect(() => {
    const serviceListWithImg = services
      .filter((s) => s.withImage)
      .sort((a, b) => a.order - b.order);
    setServiceData(serviceListWithImg);
  }, [services, isImageSlide]);
  ...
  return (
    <div className={`${styles.container}`}>
      <div
        className={
          isImageSlide
            ? `${styles.serviceDiv} ${styles.slideContainer}`
            : `${styles.serviceDiv} ${styles.serviceGrid}`
        }
      >
        <ul
          className={
            serviceData.length % 2 === 0
            ...
            serviceData.map((s, index) => (
              <li
                className={
                  s.withDescription
                    ? `${styles.lists}`
                    : `${styles.serviceNoDescription} ${styles.lists}`
                }
                key={index}
              >
                ...
                {isImageSlide ? (
                  <div
                    ref={sliderRef}
                    className={`keen-slider ${styles.slideDiv}`}
                  >
                  ...
                  <div
                    ...
                    style={{
                      backgroundImage: `url(${urlFor(s.serviceImgs[0])})`,
                    }}
                  ></div>
                )}
              </li>
